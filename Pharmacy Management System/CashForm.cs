...
namespace Pharmacy_Management_System
{
    public partial class CashForm : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            product.uname = mainForm.lblUsername.Text;
            product.ShowDialog();
        }
        ...
        private void btnCash_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure you want to cash this product?", "Cashing", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    dbcon.executeQuery("UPDATE tbProduct SET pqty= pqty - " + dgvCash.Rows[i].Cells[4].Value.ToString() + " WHERE pcode LIKE " + dgvCash.Rows[i].Cells[2].Value.ToString() + "");
                }
                dgvCash.Rows.Clear();
            }
        }
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to remove this item from the cart?", "Delete cart", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.executeQuery("DELETE FROM tbCash WHERE cashid LIKE'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Cart record has been successfully removed", title, MessageBoxButtons.OK, MessageBoxIcon.Question);
                }
            }
            else if (colName == "Increase")
            {
                ...
                {
                    MessageBox.Show("Remaining quantity on hand is " + i + "!", "Out of stock", MessageBoxButtons.OK, MessageBoxIcon.Question);
                    return;
                }
            }
        ...
        public void loadCash()
        {
            try
            {
                ...
                double total = 0;
                dgvCash.Rows.Clear();
                cm = new SqlCommand("SELECT cashid,pcode,pname,qty,price,total,c.name,cashier from tbCash as cash LEFT JOIN tbCustomer c ON cash.cid = c.cid WHERE transno LIKE " + lblTransno.Text + "", cn);
                cn.Open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
